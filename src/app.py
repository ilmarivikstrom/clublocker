...
from utils.extraction import fetch_and_save_tournaments, fetch_and_save_tournament_matches, fetch_rankings, load_pickle
...
header_container.info(f"Tournament data covers {len(tournaments_filtered)} tournaments starting from {str(tournaments_df['StartDatePandas'].min().date())} and ending in {str(tournaments_df['StartDatePandas'].max().date())}.")
...
tournament_container.write('Data includes all tournaments that are (publicly) visible in Club Locker history of Finnish Squash Association. For clarity, all canceled tournaments are excluded from the dataset.')
tournament_container.write('### Participation over time')
tournament_container.write('Let\'s examine the general tournament participation over time:')
fig, ax = plt.subplots()
fig.tight_layout()
...
tournament_container.write('Data seems to suggest that a decreasing trend in tournament participation was in place already during pre-covid era. This is indicated by the slowly decreasing blue line. Summer breaks and lockdown periods are visible as temporal gaps in the data.')
...
match_container.write('Match data includes only matches played in tournaments. League matches are not included in the data yet.')
match_container.write('### Match length distribution')
match_container.write('It is expected that the number of rallies per match is not distributed in a Gaussian manner. This is assumed because the number of games in each match vary between 3 and 5. Each additional game adds a minimum of 11 points to the total number of rallies per match.')
...
sn.kdeplot(ax=ax, data=matches_df, x='Rallies', bw_adjust=.5, palette=custom_palette_cmap)
match_container.pyplot(fig)


match_container.write('Let\'s also see how the number of rallies correlate with the match length in minutes:')
fig, ax = plt.subplots()
ax.set_xlabel('Match duration (minutes)')
ax.set_ylabel('Number of rallies')
g = sn.scatterplot(data=matches_df.loc[matches_df['NumberOfGames'] >= 3], x='MatchDuration', y='Rallies', hue='NumberOfGames', alpha=0.7, palette=custom_palette_cmap)
g.set(xlim=(0,90))
match_container.pyplot(fig)
...
selected_tournament = match_container.selectbox('Select the tournament name', tournaments_filtered['TournamentName'].loc[tournaments_filtered['Type'] == 'results'])
...
match_container.write(f'Selected {len(selected_matches)} matches from {selected_tournament}')
fig, ax = plt.subplots()
fig.tight_layout()
ax.set_xlabel('Time')
ax.set_ylabel('Number of Rallies')
...
player_container.dataframe(search_results[['matchid', 'hPlayerName', 'vPlayerName', 'Score_Short', 'Winner', 'Rallies']])
player_container.caption(f'Found {len(search_results)} matches for player {name}')


player_container.write('And here\'s the visualization of wins and losses of the selected player\'s matches:')
fig, ax = plt.subplots()
fig.tight_layout()
