...
def app():
    ...
    def fetch_data():
        headers = {
            "Authorization": "Bearer "
        }
        # Requires auth.
        boxes_response = requests.get(
            url="https://api.ussquash.com/resources/res/box_leagues/list?TopRecords=50&clubId=10209",
            headers=headers,
        )
        ...
        box_ids = (
            ...
            .loc[
                (
                    pd.to_datetime(boxes_df["startDate"])
                    > pd.to_datetime(f"{year}-01-01T00:00:00")
                )
                & (
                    pd.to_datetime(boxes_df["endDate"])
                    < pd.to_datetime(f"{year+1}-01-01T00:00:00")
                )
            ]
            .unique()
            .tolist()
        ...
        for box_id in box_ids:
            players_response = requests.get(
                url=f"https://api.ussquash.com/resources/res/box_leagues/{box_id}/players",
                headers=headers,
            )
            players_df = pd.read_json(players_response.text)
            standings_response = requests.get(
                url=f"https://api.ussquash.com/resources/res/box_leagues/{box_id}/standings",
                headers=headers,
            )
            standings_df = pd.read_json(standings_response.text)
            results_response = requests.get(
                url=f"https://api.ussquash.com/resources/res/box_leagues/{box_id}/results"
            )
            results_df = pd.read_json(results_response.text)
            one_box_contents = {
                "box": boxes_df.loc[boxes_df["eventId"] == box_id],
                "players": players_df,
    ...
    for history in range(len(boxes)):
        standings_df = pd.concat(
            [standings_df, boxes[history]["standings"]], ignore_index=True
        )
        results_df = pd.concat(
            [results_df, boxes[history]["results"]], ignore_index=True
        )

    standings_df = standings_df.groupby(by=["PlayerName"]).sum()
    standings_df["Name"] = standings_df.index

    standings_df["Win_Percentage"] = (
        round(
            standings_df["Wins_Season"]
            / (standings_df["Wins_Season"] + standings_df["Loses_Season"])
            * 100,
            1,
        )
        .astype(str)
    ...
    standings_table = standings_df.sort_values(by=["Points_Season"], ascending=False)[
        ["Name", "Wins_Season", "Loses_Season", "Points_Season", "Win_Percentage"]
    ]
    standings_table["Rank"] = np.arange(len(standings_table)) + 1
    standings_table = standings_table.rename(
        columns={
            "Rank": "Rank",
            "Wins_Season": "Wins",
            "Loses_Season": "Losses",
    ...
    standings_table = standings_table.reset_index().set_index(
        np.arange(len(standings_table)) + 1
    )

    standings_table = standings_table[
        ["Rank", "Name", "Wins", "Losses", "Points", "Win Percentage"]
    ]
